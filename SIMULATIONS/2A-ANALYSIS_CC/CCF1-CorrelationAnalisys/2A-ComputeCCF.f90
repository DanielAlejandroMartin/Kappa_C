...
integer,parameter:: SystemW=500 !Size Over which I record activity (We used 500 on the article)
...
integer,parameter:: t_end=10000  !We used 100000 in the article
integer,parameter:: t_interval=20  !We used 100000 in the article
end module
...
call system ('mkdir -p ResultsCorrel')

open(103,file='../../1-SIMULATION/1-DynamicsGenerator/ResultsState_AlreadyComputed/StateRun' &
& // trim(adjustl(file_name))// '-Tr0.'  &
& // trim(adjustl(file_name2))// '.dat' , access='DIRECT', recl=250000)


!open(103,file='../../1-SIMULATION/1-DynamicsGenerator/ResultsState/StateRun' & 
!& // trim(adjustl(file_name))// '-Tr0.'  &
!& // trim(adjustl(file_name2))// '.dat' , access='DIRECT', recl=250000)
...
open(203,file='ResultsCorrel/CorrelRun' // trim(adjustl(file_name))// '-Tr0.'  &
...
!Compute the total activity in this snapshot,setting state to 1 if neuron if active or refractary
activity_density=0
do x1=startx,startx+Window-1 ; do y1=starty,starty+Window-1
if (element_state(x1,y1)>0) then
activity_density=activity_density+1
